...
void		del_set_node_sec(t_env **env_s, char **token,
				char **type, t_env **node);
...
void		del_set_node_tr(t_env **env_s, char **token,
				char **type, t_env **node);
int			copy_split_words(char **split, char **args, int i);
int			handle_expanded_arg(char *expanded, t_cmd *cmd, int i,
				t_expansion *exp);
int			handle_split_args(char **split, t_cmd *cmd, int i,
				int org_count);
...
int			extr_args(t_token **token, t_cmd *cmd);
int			expand(t_ms *ms);
int			handle_split(t_cmd *cmd, char **split, int i,
				int original_count);
int			handle_expanded(t_cmd *cmd, char *expanded,
				int i, t_expansion *exp);
...
int			process_var_expasion_first(char *str, int *i,
				char **ex_str, t_ms *ms);
...
char		**ft_generate_args(char **n_as, char **as,
				char **s_as, const int *n);
char		*ft_strnjoin(char *s1, const char *s2, size_t n);
...
char		*handle_dollar(char *str, size_t *i,
				t_expansion *exp, char **result);
char		*handle_double_quotes(char *str, size_t *i,
				char *result, t_expansion *exp);
char		*handle_regular_char(char *str, size_t *i, char *result);
char		**allocate_new_args(char **args, const char *temp,
				char **split_args, int *n);
